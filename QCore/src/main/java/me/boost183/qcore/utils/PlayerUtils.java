package me.boost183.qcore.utils;

import me.boost183.qcore.QCore;
...
public class PlayerUtils {
    ...
    public static void matchOfflinePlayerAsync(String paramString, boolean paramBoolean, Consumer<OfflinePlayer> paramConsumer){
        Bukkit.getScheduler().runTaskAsynchronously(QCore.getInstance() , () -> {
            final OfflinePlayer offlinePlayer = matchOfflinePlayer(paramString);
            if(offlinePlayer == null || !offlinePlayer.hasPlayedBefore()) {
                Bukkit.getScheduler().runTask(QCore.getInstance(), () -> paramConsumer.accept(null));
                return;
            }
            if(!offlinePlayer.isOnline()) {
                if (paramBoolean)
                    Bukkit.getScheduler().runTask(QCore.getInstance(), () -> paramConsumer.accept(offlinePlayer));
                else Bukkit.getScheduler().runTask(QCore.getInstance(), () -> paramConsumer.accept(null));
                return;
            }
            Bukkit.getScheduler().runTask(QCore.getInstance(), () -> paramConsumer.accept(offlinePlayer));
        });
